function initQuery() {
    $("#list").datagrid({
        ...
        pagination: true,
        columns: [[
            { field: 'areano', title: '区域代码' },
            { field: 'areaname', title: '区域名称' },
            { field: 'areaorder', title: '排序' }
        ]]
    });
}
...
function initShowAdd() {
    $("#btnShowAdd").click(function () {
        showDialog("editframe", "/Area/Edit?a=1" + Math.random(), {
            height: 200,
            buttons: [{
                text: '保存',
                iconCls: 'icon-ok',
...
function initShowUpdate() {
    $("#btnShowEdit").click(function () {
        ...
        if (row) {
            var areano = row.areano;
            showDialog("editframe", "/Area/Edit?a=1" + Math.random() + "&areano=" + areano, {
                buttons: [{
                    text: '保存',
                    iconCls: 'icon-ok',
                    handler: function () {
    ...
    $("#btnDelete").click(function () {
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                var row = $("#list").datagrid("getSelected");
                if (row) {
                    $.post("/area/delete", { areano: row.areano, rnd: Math.random() }, function (data) {
...
function showUpdateResult(data) {
    if (data && data.success === true) {
        if ($("#editform").attr("action").toLowerCase() == "/area/add") {
