function initQuery() {
    $("#btnQuery").click(function () {
        ...
        $("#list").datagrid({
            url: '/Xueyuan/Index',
            queryParams: { xueyh: xueyh, xingming: xingming, shenfhm: shenfhm, rnd: Math.random() },
            cls: 'table-wrap',
            fit: true,
            rownumbers: true,
            singleSelect: true,
...
function initShowAdd() {
    $("#btnShowAdd").click(function () {
        showDialog("editframe", "/Xueyuan/Edit?a=1" + Math.random(), {
            height: 330,
            buttons: [{
                text: '保存',
                iconCls: 'icon-ok',
...
function initShowUpdate() {
    $("#btnShowEdit").click(function () {
        ...
        if (row) {
            var xueyh = row.xueyh;
            showDialog("editframe", "/Xueyuan/Edit?a=1" + Math.random() + "&xueyh=" + xueyh, {
                buttons: [{
                    text: '保存',
                    iconCls: 'icon-ok',
                    handler: function () {
    ...
    $("#btnDelete").click(function () {
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                var row = $("#list").datagrid("getSelected");
                if (row) {
                    $.post("/Xueyuan/delete", { xueyh: row.xueyh, rnd: Math.random() }, function (data) {
...
function showUpdateResult(data) {
    if (data && data.success === true) {
        if ($("#editform").attr("action").toLowerCase() == "/xueyuan/add") {
